...
/**
 * Convert DeepSeek messages to common format for trusted data evaluation
 */
export function toCommonFormat(messages: DeepSeekMessages): CommonMessage[] {
...
/**
 * Apply tool result updates back to DeepSeek messages
 */
...
/**
 * Extract the user's original request from DeepSeek messages
 */
export function extractUserRequest(messages: DeepSeekMessages): string {
  const userContent =
...
/**
 * Convert DeepSeek tool calls to common format for MCP execution
 */
...
/**
 * Convert common tool results to DeepSeek tool message format
 */
export function toolResultsToMessages(
  ...
  return results.map((result) => {
    ...
    if (result.isError) {
      ...
      content = toonEncode(result.content);
    } else {
